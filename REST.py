...
class REST():
    ...
    def get(self, path):
        ...
        connection = self.sessions.get(
            self.protocol+'://'+self.url+path, proxies=self.proxies, timeout=self.timeout)
        return connection
    ...
    def post(self, path, payload=None):
        ...
        connection = self.sessions.post(
            self.protocol+'://'+self.url+path, json=payload, proxies=self.proxies, timeout=self.timeout)
        return connection
    ...
    def put(self, path, payload):
        ...
        connection = self.sessions.put(
            self.protocol+'://'+self.url+path, data=payload, proxies=self.proxies, timeout=self.timeout)
        return connection
